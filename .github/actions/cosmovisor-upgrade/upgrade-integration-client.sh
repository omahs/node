...
cp "$GOPATH"/bin/old/zetaclientd "$DAEMON_HOME"/cosmovisor/genesis/bin/
cp "$GOPATH"/bin/new/zetaclientd "$DAEMON_HOME"/cosmovisor/upgrades/"$UpgradeName"/bin/
...
chmod +x "$DAEMON_HOME"/cosmovisor/upgrades/"$UpgradeName"/bin/zetaclientd

contents="$(jq '.app_state.gov.voting_params.voting_period = "90s"' /home/zetachain/.zetacored/config/genesis.json)" && \
echo "${contents}" > /home/zetachain/.zetacored/config/genesis.json
...
zetacored tx gov submit-proposal software-upgrade test1 --from val --deposit 10000stake --upgrade-height 10 --title test1 --description description --keyring-backend test --chain-id athens-1 --yes --home ~/.zetacored/
